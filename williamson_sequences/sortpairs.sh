...
do
	...
	then
		START=$(date +%s.%N)
		sort -S 5G -T tmp/ matchings/$n.$c.$l.AB.pafs.txt > matchings/$n.$c.$l.AB.pafs.sorted.txt
		END=$(date +%s.%N)
	...
	then
		START=$(date +%s.%N)
		sort -S 5G -T tmp/ matchings/$n.$c.$l.CD.pafs.txt > matchings/$n.$c.$l.CD.pafs.sorted.txt
		END=$(date +%s.%N)
		DIFF=$(echo "$END - $START" | bc)
