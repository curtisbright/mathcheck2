...
resultname = "results/compprogassum/%s.out"
logname = "output/compprogassum/%s.log"

if not os.path.exists("output"): call(["mkdir", "output"])
if not os.path.exists("output/compprogassum"): call(["mkdir", "output/compprogassum"])
if not os.path.exists("results"): call(["mkdir", "results"])
if not os.path.exists("results/compprogassum"): call(["mkdir", "results/compprogassum"])
...
for n in range(n1, n2+1):
    ...
    for f in files:
        ...
        count += 1
        command = "./maplesat_static {0} {1} -no-pre -xnormult -cardinality -order={2} -carda={3} -cardb={4} -cardc={5} -cardd={6} -compsums={7} -assumptions={8}".format(inname % runstr(n,c,-1), resultname % runstr(n, c, k), n, sq[n][c][0]*(-1 if sq[n][c][0] % 4 == 3 else 1), sq[n][c][1]*(-1 if sq[n][c][1] % 4 == 3 else 1), sq[n][c][2]*(-1 if sq[n][c][2] % 4 == 3 else 1), sq[n][c][3]*(-1 if sq[n][c][3] % 4 == 3 else 1), compname % runstr(n, c, k), assumname % runstr(n, c, k))
        if sharcnet == True:
