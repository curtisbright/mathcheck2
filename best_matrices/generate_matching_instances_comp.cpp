...
#define MAX_N 70
#define REMOVE_EQUIV_A
#define REMOVE_EQUIV_B
...
int main(int argc, char** argv)
{
	...
	FILE *Aseqnsfile, *Bseqnsfile, *Dseqnsfile[4];
	FILE *Apafsfile, *Bpafsfile, *Dpafsfile[4];
	int Dtarget[4];
	...
	sprintf(filename, seqnsfilename, n, 0, l, 'B');
	Bseqnsfile = fopen(filename, "w");
	sprintf(filename, pafsfilename, n, 0, l, 'B');
	Bpafsfile = fopen(filename, "w");
	...
	clock_t start = clock();
	int Acount = 0, Bcount = 0, Dcount[4] = {};
	...
	{	
		...
		{	
			...
			{	
				...
				#ifdef REMOVE_EQUIV_B
				std::array<int, MAX_N> permutedcomp = permuteA(l, l-1, compressA);
				if(myset_A.count(permutedcomp)!=0)
					toadd_B = false;
				#endif
				...
				if(toadd_B == true)
				{	fprintseqn(Bseqnsfile, l, compressA.data());
					fprintpafs(Bpafsfile, l, compressA.data());
					Bcount++;
				}
	...
	for(int c=0; c<decomps_len[n]; c++)
		printf("    Case (%d): %d As, %d Bs, %d Cs, %d Ds\n", Dtarget[c], Acount, Bcount, Bcount, Dcount[c]);
