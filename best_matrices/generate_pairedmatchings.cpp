...
int main(int argc, char** argv)
{
	...
	sprintf(filename, pafsfilename, n, 0, l, "AB");
	...
	fclose(pairfile);
	printf("  Computed AB PSDs in %.2f seconds\n", (clock() - start)/(float)CLOCKS_PER_SEC);
	#endif
	...
	for(int c = 0; c < decomps_len[n]; c++)
	{
		...
		{	
			...
			{	
				...
				for(int i = 0; i < pafslen; i++)
				{	CDpafs[i] = - (Bpafs[iB][i] + Dpafs[iD][i]);
					if(i==0)
						CDpafs[i] += 4*n;
				}

				fprintpair(pairfile, pafslen, CDpafs.data(), iB, iD);
				CDcount++;
			}
		}
