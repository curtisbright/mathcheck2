...
{	
	...
	{	int k = coprimelist[n][j];
		...
		std::array<int, MAX_N> altpermutedA = permuteA(l, l-1, 0, permutedA);
		std::array<int, MAX_N> altpermutedB = permuteA(l, l-1, 0, permutedB);
		std::array<int, MAX_N> altpermutedC = permuteA(l, l-1, 0, permutedC);
		...
		#if !SMALLCLASSES
		if(altpermutedA<permutedA)
			permutedA = altpermutedA;
		if(altpermutedB<permutedB)
			permutedB = altpermutedB;
		if(altpermutedC<permutedC)
...
int main(int argc, char** argv)
{
	...
	const char seqnsprettyoutfilename[] = "exhaust/best-%d.txt";
	...
	for(int c = 0; c < decomps_len[n]; c++)
	{
		...
		{	
			...
			std::tuple<std::array<int, MAX_N>, std::array<int, MAX_N>, std::array<int, MAX_N>, std::array<int, MAX_N>> repseqn = minrep(n, n, A, B, C, D);
		...
		if(invalidcount > 0)
			printf("  WARNING: Read %d sequences which were not defining rows of best matrices\n", invalidcount);
	}
